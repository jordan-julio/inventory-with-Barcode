...
export default function AddUser() {
    ...
    useEffect(() => {
        ...
        const getOneCategoryData = async (id: string) => {
            try {
                const response = await axios.get(`http://localhost:3001/admins/api/getOneCategory/${id}`);
                form.setFieldsValue({
    ...
    const handleSubmit = async (values: any) => {
        try {
            // hash password
            const response = await axios.put("http://localhost:3001/admins/api/editOneCategory", { id: id, categoryName: values.categoryName, description: values.description});
            success();
          } catch (error) {
            console.error(error);
          }
